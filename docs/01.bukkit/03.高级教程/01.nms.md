...
```
好，我们拿到 CraftBukkit 那一层的实例了。CraftBukkit 这一层可能会有比 Bukkit 更多的*内部*方法，但通常不会有很多。一般来说，只需要访问 `handle` 字段，或者调用 `getHandle()` 方法，就能拿到 Minecraft 原版对象。不同版本的获取方法不通，有的版本还可能不会把 `handle` 字段给设置成公开的，要反射去取。
```java

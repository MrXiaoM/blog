...
接下来该轮到你发挥了，如果你从未接触过编程，查阅以下几条规则来帮你快速写出代码。

::: details
学习一门新编程语言的宗旨是，先了解基本语法结构，遇到不会的东西就去网上查，去问人。代码抄得多了你就能够逐渐了解为什么要这么写了。尽量少问 GPT，AI 或许能很快地给出答案，但答案的正确率不一定比人高，优先寻找人类编写的帖子，最好加入一个技术交流群。只要不是太过于低级的、网上能查到的问题，程序员们通常乐于回复你的问题。
...
在 Java 中，以 `//` 开头的是注释 (字体会显示为绿色或灰色)，注释不参与代码编译，仅仅是给人阅读的一串说明，之后的代码会写一些注释以方便读者理解。
...
把它想象成一扇门，我们可以在它后面加一个 `.` 来打开它，进入它后面的房间。  
